...
const AnimatePresence = class {
    constructor(hostRef) {
        ...
        this.getClosestParent = () => {
            ...
            return index.closest(this.element.tagName, base);
        };
        this.willExit = false;
        this.didExit = false;
        ...
        this.animatePresenceExitComplete = core.createEvent(this, "animatePresenceExitComplete", 7);
        this.animatePresenceEnter = core.createEvent(this, "animatePresenceEnter", 7);
        this.animatePresenceExit = core.createEvent(this, "animatePresenceExit", 7);
    }
    observeChanged() {
        if (this.observe) {
            this.addMO();
    ...
    async handleEnter(node, _record, i) {
        if (!index.isHTMLElement(node))
            return;
        if (index.hasData(node, 'exit'))
            return;
        if (index.hasData(node, 'willExit')) {
    ...
    async handleExit(node, record, i) {
        if (!index.isHTMLElement(node))
            return;
        if (index.hasData(node, 'exit') || index.hasData(node, 'willExit')) {
            return;
        }
        node.dataset.willExit = '';
        index.setCustomProperties(node, { i });
        if (index.isHTMLElement(record.previousSibling)) {
            record.previousSibling.insertAdjacentElement('afterend', node);
        }
        else if (index.isHTMLElement(record.target)) {
    ...
    async enter() {
        ...
        await index.enterChildren(this.element);
        this.didEnter = true;
        this.willEnter = false;
        return Promise.resolve();
    }
    render() {
        return (core.h(core.Host, { style: { display: 'contents' } }, core.h("slot", null)));
    }
